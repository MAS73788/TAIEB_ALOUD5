    ...
    <div class="border border-gray-200 rounded-2xl p-4 bg-white shadow-sm">
      <div class="flex items-start justify-between flex-wrap gap-2">
        <div>
          ...
          ${o.isGift ? `<div class="text-xs text-pink-600 mt-1">🎁 هدية${o.giftMessage ? ' — "' + escapeHTML(o.giftMessage) + '"' : ''}${o.giftWrapFee ? ` (رسم تغليف: ${o.giftWrapFee})` : ''}</div>` : ''}
        </div>
        <div class="text-left">
...
// ============================================================
// FEATURES: تشغيل/إيقاف ميزات الموقع (بحث، فلتر، ترتيب، منتجات مشابهة،
// تتبع الطلب، تغليف الهدايا) من غير أي تعديل كود
// ============================================================
async function loadFeatureSettings() {
  try {
    const snap = await getDoc(doc(db, 'settings', 'features'));
    const d = snap.exists() ? snap.data() : {};
    document.getElementById('feat_search').checked = d.searchEnabled !== false;
    document.getElementById('feat_filter').checked = d.filterEnabled !== false;
    document.getElementById('feat_sort').checked = d.sortEnabled !== false;
    document.getElementById('feat_related').checked = d.relatedEnabled !== false;
    document.getElementById('feat_trackOrder').checked = d.trackOrderEnabled !== false;
    document.getElementById('feat_giftWrap').checked = !!d.giftWrapEnabled;
    document.getElementById('feat_giftWrapFee').value = d.giftWrapFee || 0;
  } catch (err) {
    console.error(err);
    showToast('تعذر تحميل إعدادات الميزات: ' + err.message, 'error');
  }
}
...
async function saveFeatureSettings() {
  ...
  try {
    await setDoc(doc(db, 'settings', 'features'), {
      searchEnabled: document.getElementById('feat_search').checked,
      filterEnabled: document.getElementById('feat_filter').checked,
      sortEnabled: document.getElementById('feat_sort').checked,
      relatedEnabled: document.getElementById('feat_related').checked,
      trackOrderEnabled: document.getElementById('feat_trackOrder').checked,
      giftWrapEnabled: document.getElementById('feat_giftWrap').checked,
      giftWrapFee: parseFloat(document.getElementById('feat_giftWrapFee').value) || 0,
      updatedAt: serverTimestamp()
    }, { merge: true });
    showToast('تم حفظ الميزات ✅');
  } catch (err) {
    console.error(err);
    showToast('تعذر حفظ الميزات: ' + err.message, 'error');
  } finally {
    btn.disabled = false;
  }
}
...
function setupFeaturesTab() {
  const btn = document.getElementById('saveFeaturesBtn');
  if (btn) btn.addEventListener('click', saveFeatureSettings);
}
